...
export default ({ database, className }: Props) => {
    ...
    const schema = object().shape({
        confirm: string()
            ...
            .oneOf([database.name.split('_', 2)[1], database.name], 'Nazwa bazy danych jest wymagana.'),
    });
    ...
    return (
        <>
                    ...
                    >
                        ...
                        <Form css={tw`m-0 mt-6`}>
                            <Field
                                ...
                                id={'confirm_name'}
                                name={'confirm'}
                                label={'Potwierdź nazwę bazy danych'}
                                description={'Wpisz nazwę bazy danych, aby potwierdzić usunięcie.'}
                            />
            ...
            <Modal visible={connectionVisible} onDismissed={() => setConnectionVisible(false)}>
                ...
                <h3 css={tw`mb-6 text-2xl`}>Szczegóły połączenia z bazą danych</h3>
                <div>
            ...
            <GreyRowBox $hoverable={false} className={className} css={tw`mb-2`}>
                ...
                <div css={tw`ml-8 text-center hidden md:block`}>
                    ...
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>Adres końcowy</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <p css={tw`text-sm`}>{database.allowConnectionsFrom}</p>
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>Połączenia z</p>
                </div>
                <div css={tw`ml-8 text-center hidden md:block`}>
                    <CopyOnClick text={database.username}>
                        <p css={tw`text-sm`}>{database.username}</p>
                    </CopyOnClick>
                    <p css={tw`mt-1 text-2xs text-neutral-500 uppercase select-none`}>Nazwa użytkownika</p>
                </div>
                <div css={tw`ml-8`}>
