    ...
    return (
        <GreyRowBox css={tw`mb-2`}>
            ...
            <div css={tw`ml-4`}>
                ...
                <p css={tw`text-2xs text-neutral-500 uppercase hidden md:block`}>2FA Aktywne</p>
            </div>
            <div css={tw`ml-4 hidden md:block`}>
                <p css={tw`font-medium text-center`}>
                    {subuser.permissions.filter((permission) => permission !== 'websocket.connect').length}
                </p>
                <p css={tw`text-2xs text-neutral-500 uppercase`}>Uprawnienia</p>
            </div>
            {subuser.uuid !== uuid && (
                <>
                    <Can action={'user.update'}>
                        <button
                            type={'button'}
                            aria-label={'Edytuj subużytkownika'}
