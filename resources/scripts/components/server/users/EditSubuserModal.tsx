    ...
    return (
        <Formik
            ...
            validationSchema={object().shape({
                email: string()
                    .max(191, 'Adres e-mail nie może przekraczać 191 znaków.')
                    .email('Musi zostać podany poprawny adres e-mail.')
                    .required('Musi zostać podany poprawny adres e-mail.'),
            ...
            <Form>
                <div css={tw`flex justify-between`}>
                    <h2 css={tw`text-2xl`} ref={ref}>
                        {subuser
                            ? `${canEditUser ? 'Modyfikuj' : 'Podgląd'} uprawnienia dla ${subuser.email}`
                            : 'Dodaj nowego subużytkownika'}
                    </h2>
                    <div>
                        <Button type={'submit'} css={tw`w-full sm:w-auto`}>
                            {subuser ? 'Zapisz' : 'Zaproś użytkownika'}
                        </Button>
                    </div>
                </div>
                ...
                {!isRootAdmin && loggedInPermissions[0] !== '*' && (
                    <div css={tw`mt-4 pl-4 py-2 border-l-4 border-cyan-400`}>
                        <p css={tw`text-sm text-neutral-300`}>
                            Możesz wybrać tylko te uprawnienia, które są obecnie przypisane do Twojego konta podczas tworzenia lub modyfikowania innych użytkowników.
                        </p>
                    </div>
                )}
                {!subuser && (
                    <div css={tw`mt-6`}>
                        <Field
                            ...
                            description={
                                'Wprowadź adres e-mail użytkownika, którego chcesz zaprosić jako subużytkownika dla tego serwera.'
                            }
                        />
                    </div>
                )}
