...
export default () => {
    ...
    return (
        <PageContentBlock title={'Harmonogramy'}>
                ...
                <>
                    ...
                    <div css={tw`rounded shadow`}>
                        ...
                        >
                            <div css={tw`flex-1`}>
                                ...
                                <p css={tw`mt-1 text-sm text-neutral-200`}>
                                    ...
                                    {schedule.lastRunAt ? (
                                        format(schedule.lastRunAt, "MMM do 'o' h:mma")
                                    ) : (
                                        <span css={tw`text-neutral-300`}>brak</span>
                                    )}
                                    <span css={tw`ml-4 pl-4 border-l-4 border-neutral-600 py-px`}>
                                        Następne uruchomienie:&nbsp;
                                        {schedule.nextRunAt ? (
                                            format(schedule.nextRunAt, "MMM do 'o' h:mma")
                                        ) : (
                                            <span css={tw`text-neutral-300`}>brak</span>
                                        )}
                                    </span>
                        ...
                        <div css={tw`hidden sm:grid grid-cols-5 md:grid-cols-5 gap-4 mb-4 mt-4`}>
                            <CronBox title={'Minuta'} value={schedule.cron.minute} />
                            <CronBox title={'Godzina'} value={schedule.cron.hour} />
                            <CronBox title={'Dzień (miesiąc)'} value={schedule.cron.dayOfMonth} />
                            <CronBox title={'Miesiąc'} value={schedule.cron.month} />
                            <CronBox title={'Dzień (tydzień)'} value={schedule.cron.dayOfWeek} />
                        </div>
