...
export default () => {
    ...
    return (
        <PageContentBlock title={'Klucze SSH'}>
            <FlashMessageRender byKey={'account'} />
            <div css={tw`md:flex flex-nowrap my-10`}>
                <ContentBox title={'Dodaj klucz SSH'} css={tw`flex-none w-full md:w-1/2`}>
                    <CreateSSHKeyForm />
                </ContentBox>
                <ContentBox title={'Klucze SSH'} css={tw`flex-1 overflow-hidden mt-8 md:mt-0 md:ml-8`}>
                    <SpinnerOverlay visible={!data && isValidating} />
                    {!data || !data.length ? (
                        <p css={tw`text-center text-sm`}>
                            {!data ? 'Ładowanie...' : 'Brak kluczy SSH dla tego konta.'}
                        </p>
                    ) : (
                        data.map((key, index) => (
                            <GreyRowBox
